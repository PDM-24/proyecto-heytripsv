...
private fun isoDateFormat(dateToFormat: String): String{
    try {
...
private fun timeToIso (time: String): String{
    ...
    Log.i("view", time)
    if (time.trim() == ""){
        val hour = LocalTime.parse("00:00", DateTimeFormatter.ofPattern("hh:mm a", Locale.ENGLISH))
...
@Composable
private fun TimeDialog(onConfirm: ()->Unit, state: TimePickerState, onDismissRequest: ()->Unit){
